...
**与 `utils`/`qiankun-shared` 的区别：** 同样是 `packages/` 下的内部包，但消费方式决定了是否需要 `.ts` 扩展名：

| 包                        | 消费方                       | 模块解析方式           |
| ------------------------- | ---------------------------- | ---------------------- |
| `eslint-config`           | Node.js ESM（ESLint 运行时） | 严格要求完整路径       |
| `utils`、`qiankun-shared` | Vite（bundler）              | bundler 自动补全扩展名 |
...
```jsonc [packages/qiankun-shared/tsconfig.json]
{
  "extends": "../../tsconfig.base.json",
  "include": ["src/**/*"],
}
