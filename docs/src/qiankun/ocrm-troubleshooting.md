...
- 构建脚本：`core-build-local/scripts/build-qiankun.js`
- Webpack 初始化配置：`core-build-local/scripts/webpack.init.js`
- OCRM 子应用入口：`core-fooms/projects/candao-ocrm/index/config.js`
...
- 入口文件没有按 qiankun 约定导出 `bootstrap / mount / unmount`，或者实际入口并不是 `core-fooms/projects/candao-ocrm/index/config.js` 第 698 行附近这一份。

当前代码里，这两部分都已经补齐：

- `core-build-local/scripts/build-qiankun.js` 第 6 行附近将产物声明为 `library: 'ocrm'`、`libraryTarget: 'umd'`
- `core-fooms/projects/candao-ocrm/index/config.js` 第 706 行附近明确导出了 `mount`
- `core-fooms/projects/candao-ocrm/index/config.js` 第 791 行附近明确导出了 `unmount`
- `core-fooms/projects/candao-ocrm/index/config.js` 第 797 行附近明确导出了 `bootstrap`
...
OCRM 的 Webpack 默认配置在 `core-build-local/scripts/webpack.init.js` 第 12 行附近是：
...
- `core-fooms/projects/candao-ocrm/index/config.js` 第 698 行附近的 `__webpack_public_path__` 注入
- `core-build-local/scripts/build-qiankun.js` 第 14 行附近对 loader `publicPath` 的删除
