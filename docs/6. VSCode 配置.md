...
- Windows 默认使用 CRLF (`\r\n`)
- macOS/Linux 使用 LF (`\n`)
- 此配置确保提交到 Git 的文件统一使用 LF

#### 与 .editorconfig 的配合

```
.editorconfig   →  end_of_line = lf    (编辑器保存时使用 LF)
.gitattributes  →  eol=lf             (Git 提交时强制使用 LF)
```

两者配合使用，确保跨平台开发时换行符一致，避免 Git diff 显示大量换行符变更。
...
- **`.gitattributes`**: Git 换行符处理（确保跨平台一致性）
